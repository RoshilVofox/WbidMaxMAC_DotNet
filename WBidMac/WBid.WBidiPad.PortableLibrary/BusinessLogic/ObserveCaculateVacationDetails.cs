...
{
    public class ObserveCaculateVacationDetails
    {
                            ...
                            foreach (Absense vacPerid in GlobalSettings.OrderedVacationDays)
                            {
                                ...
                                if (vacPerid.AbsenceType == "VA")
                                {
                                    ...
                                    if (tripStartDate >= vacPerid.StartAbsenceDate && tripEndDate <= vacPerid.EndAbsenceDate)
                                    {
                                        ...
                                        {
                                            int numberofRedEyeDayinBP = GetNumberofEmptyDutyperiodsInsideRedEyeTrip(tripStartDate, trip);
                                            line.Value.VacationStateLine.VacationDaysOff += numberofRedEyeDayinBP;
                                        }
                                                                                                  ...
                                                                                                  ((vacPerid.StartAbsenceDate.AddDays(-1) == tripEndDate) && (tripEndTime > 1440)))
                                    {
                                        ...
                                        //Roshil added 29-5-2025
                                        line.Value.VacationStateLine.VacationFrontNextBp += vacTrip.VacationGrandTotal.VO_TfpInLineTot - vacTrip.VacationGrandTotal.VO_TfpInBpTot;
                                    ...
                                    else if (tripStartDate <= vacPerid.EndAbsenceDate && tripEndDate > vacPerid.EndAbsenceDate)
                                    {
                                        ...
                                        //Roshil added on 22-5-2025 to get the next BP VOB values for calculating the clawback
                                        line.Value.VacationStateLine.VacationBacknextBp += vacTrip.VacationGrandTotal.VO_TfpInLineTot - vacTrip.VacationGrandTotal.VO_TfpInBpTot;
                                        line.Value.VacationStateLine.VacationDaysOff += vacTrip.DutyPeriodsDetails.Where(x => x.isInBp && (x.VacationType == "VO" || x.VacationType == "VA")).Count();
                                        if (trip.RedEye && trip.DutyPeriods.Count != trip.PairLength)
                                        {
                                            int numberofRedEyeDayinBP = GetNumberofEmptyDutyperiodsInsideRedEyeTrip(tripStartDate, trip);
        ...
        #endregion
        private static int GetNumberofEmptyDutyperiodsInsideRedEyeTrip(DateTime tripStartDate, Trip trip)
        {
            int numberofRedEyeDayinBP = 0;
            ...
            foreach (var item in misseddutyday)
            {
                ...
                if (GlobalSettings.CurrentBidDetails.BidPeriodStartDate <= redEyeDPDate && redEyeDPDate <= GlobalSettings.CurrentBidDetails.BidPeriodEndDate)
                    numberofRedEyeDayinBP++;
            }

            return numberofRedEyeDayinBP;
        }
        ...
        private void CalculateVAP(List<TempVap> tempVapList, KeyValuePair<string, Line> line)
        {
            ...
            try
            {
                ...
                foreach (Absense vacPerid in GlobalSettings.OrderedVacationDays)
                {
                    ...
                    //Roshil add on 18-5-2025 : We need to set VAP for the next BP is zero for all lines (FOR EOM Vacation only)
                    if (vacPerid.StartAbsenceDate > GlobalSettings.CurrentBidDetails.BidPeriodEndDate)
                        actualVapInNextBp = 0;
...
vacPerid.StartAbsenceDate < GlobalSettings.CurrentBidDetails.BidPeriodStartDate)
        ...
        private decimal CalculateFlyPay(DateTime tripStartDate, Trip trip, KeyValuePair<string, Line> line)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        if (tripStartDate.AddDays(dp.DutyDaySeqNum - 1) <= GlobalSettings.CurrentBidDetails.BidPeriodEndDate)
                        {
                            tfp += dp.Tfp;
                            pairLength++;
        ...
        private void SetInitialValues(KeyValuePair<string, Line> line)
        {
            ...
            line.Value.VacationStateLine.VacationBacknextBp = 0.0m;
            line.Value.VacationStateLine.VacationFrontNextBp  = 0.0m;

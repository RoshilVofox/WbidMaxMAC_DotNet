...
{
    public class ObservedPrepareModernBidLineView
    {
        ...
        private void CalculatebidLinePropertiesforVacation(Line line)
        {
            ...
            try
            {
                ...
                int paringCount = 0;
                decimal monthToMonthVaBp = 0;
                decimal monthToMonthVainNextBp = 0;
                foreach (string pairing in line.Pairings)
                {
                    ...
                    try
                    {
                        ...
                        Absense currentVacation = new Absense();
                        ...
                        {
                            currentVacation = GlobalSettings.TempOrderedVacationDays.FirstOrDefault(x => x.AbsenceType == "VA" && (x.StartAbsenceDate <= tripStartDate && x.EndAbsenceDate >= tripEndDate));
                            tripType = "VA";
                        }
                        ...
                        {
                            currentVacation = GlobalSettings.TempOrderedVacationDays.FirstOrDefault(x => x.AbsenceType == "VA" && ((x.StartAbsenceDate > tripStartDate) && (x.StartAbsenceDate <= tripEndDate)) ||
                            ((x.StartAbsenceDate.AddDays(-1) == tripEndDate) && (tripEndTime > 1440)));
                            tripType = "VOF";
                            if (GlobalSettings.TempOrderedVacationDays.Any(x => x.AbsenceType == "VA" && (x.EndAbsenceDate >= tripStartDate && x.EndAbsenceDate <= tripEndDate)))
                            {
                                //checking it overlaps EOM.
                        ...
                        {
                             currentVacation = GlobalSettings.TempOrderedVacationDays.FirstOrDefault(x => x.AbsenceType == "VA" && (x.EndAbsenceDate >= tripStartDate && x.EndAbsenceDate <= tripEndDate));
                            tripType = "VOB";
                        }
                        ...
                        if (tripType != string.Empty)
                        {
                            ...
                            if (tripType == "VA")
                            {
                                ...
                                if (vTrip != null)
                                {
                                    ...
                                    if (trip.RedEye && trip.DutyPeriods.Count != trip.PairLength)
                                    {
                                        int vtriprefdpseqNum = vTrip.DutyPeriodsDetails.LastOrDefault(x => x.VacationType == "VA" && x.isInBp).RefDutPdSeqNum;
                                        int lastdayOfRedeyDutyDayInBp = trip.DutyPeriods.FirstOrDefault(x => x.DutPerSeqNum == vtriprefdpseqNum).DutyDaySeqNum;
                                        DateTime VAEndDate = tripStartDate.AddDays(lastdayOfRedeyDutyDayInBp - 1);
                                        if (tripEndDate > GlobalSettings.CurrentBidDetails.BidPeriodEndDate)
                                        {
                                            //need to find out the middle day(RE) is in Bidperiod or next Bp. This case will happen if a red eye trip overlapped into the next month. In this case we need to find the exact VA Days in the bid period. We need to count the Emplty middle day also in case if the middle day is in Bp.
                                           int numberofEmptyVARedEyeDayInBp=(GlobalSettings.CurrentBidDetails.BidPeriodEndDate - VAEndDate).Days;
                                            if (numberofEmptyVARedEyeDayInBp > 0)
                                                VAEndDate = VAEndDate.AddDays(numberofEmptyVARedEyeDayInBp);
                                        }
                                            VAHelperList.Add(new VAHelper() { StartDate = tripStartDate, EndDate = VAEndDate, VA = vATfp, IsInBp = true });
                                    }
                                    else
                                    {
                                        VAHelperList.Add(new VAHelper() { StartDate = tripStartDate, EndDate = tripStartDate.AddDays(vTrip.DutyPeriodsDetails.Where(x => x.VacationType == "VA" && x.isInBp).Count() - 1), VA = vATfp, IsInBp = true });
                                    ...
                                    if (tripEndDate > GlobalSettings.CurrentBidDetails.BidPeriodEndDate)
                                    {
                                        //Roshil added on 23-5-2025 for red eye trip
                                        if (trip.RedEye && trip.DutyPeriods.Count != trip.PairLength)
                                        {
                                            if (vTrip.DutyPeriodsDetails.Where(x => x.VacationType == "VA" && !x.isInBp).Count() > 0)
                                            {
                                                VAHelperList.Add(new VAHelper() { StartDate = GlobalSettings.CurrentBidDetails.BidPeriodEndDate.AddDays(1), EndDate = tripEndDate, VA = vTrip.VacationGrandTotal.VA_TfpInLineTot - vTrip.VacationGrandTotal.VA_TfpInBpTot, IsInBp = true });
                                        ...
                                        else
                                        {
                                            ...
                                            if (vTrip.DutyPeriodsDetails.Where(x => x.VacationType == "VA" && !x.isInBp).Count() > 0)
                                            {
                                                VAHelperList.Add(new VAHelper() { StartDate = tripEndDate.AddDays(-dpinoutsidebidperiod + 1), EndDate = tripEndDate, VA = vTrip.VacationGrandTotal.VA_TfpInLineTot - vTrip.VacationGrandTotal.VA_TfpInBpTot, IsInBp = true });
                                    ...
                                    middleDutyperiod = (middleDutyperiod / 2) - 1;
                                    BidLines.ToList()[middleDutyperiod].ArrStaLastLegDisplay = "VA";
                                    if (currentVacation.EndAbsenceDate > GlobalSettings.CurrentBidDetails.BidPeriodEndDate)
                                    {
                                        monthToMonthVaBp += vTrip.VacationGrandTotal.VA_TfpInBpTot;
                                        monthToMonthVainNextBp += vTrip.VacationGrandTotal.VA_TfpInLineTot - vTrip.VacationGrandTotal.VA_TfpInBpTot;
                            ...
                            else if (tripType == "VOF")
                            {
                                ...
                                if (trip.RedEye)
                                {
                                   var lastdp= vTrip.DutyPeriodsDetails.LastOrDefault();
                                    if (lastdp.IsDPStartedOnPreviousSouthwestDay && lastdp.VacationType == "VO")
                                    {
                                        //we can assume that last day is VA, but its moved to VO dp for the red eye. For VAP calcualtion, we need to consider this day as VA day.
                                        vADays = vADays + 1;
                                    }
                                ...
                                if (tripEndDate > GlobalSettings.CurrentBidDetails.BidPeriodEndDate)
                                {
                                    if (trip.RedEye)
                                    {
                                        //VAHelperList.Add(new VAHelper() { StartDate = tripEndDate.AddDays(-(vADays - 1)), EndDate = tripEndDate, VA = vATfp - (vTrip.VacationGrandTotal.VA_TfpInLineTot - vTrip.VacationGrandTotal.VA_TfpInBpTot), IsInBp = true });
                                        //Roshil modified on 5-6-205 assuming that for month to month VOF trip. VA will always starts from vacation date. Or assuming that VA will be first day after the vacation date.
                                        VAHelperList.Add(new VAHelper() { StartDate = currentVacation.StartAbsenceDate, EndDate = GlobalSettings.CurrentBidDetails.BidPeriodEndDate, VA = vATfp - (vTrip.VacationGrandTotal.VA_TfpInLineTot - vTrip.VacationGrandTotal.VA_TfpInBpTot), IsInBp = true });

                                    }
                                    else
                                    {
                                        VAHelperList.Add(new VAHelper() { StartDate = tripEndDate.AddDays(-(vADays - 1)), EndDate = GlobalSettings.CurrentBidDetails.BidPeriodEndDate, VA = vATfp - (vTrip.VacationGrandTotal.VA_TfpInLineTot - vTrip.VacationGrandTotal.VA_TfpInBpTot), IsInBp = true });
                                    }
                                ...
                                else
                                {
                                    if (trip.RedEye)
                                    {
                                        //Roshil change on 23-5-2025
                                        //if (vATfp != 0)
                                        //{
                                        VAHelperList.Add(new VAHelper() { StartDate = currentVacation.StartAbsenceDate, EndDate = tripEndDate, VA = vATfp, IsInBp = true });

                                        // }
                                    }
                                    else
                                    {
                                        VAHelperList.Add(new VAHelper() { StartDate = tripEndDate.AddDays(-(vADays - 1)), EndDate = tripEndDate, VA = vATfp, IsInBp = true });
                                ...
                                foreach (VacationDutyPeriod vacationDutyPeriod in vTrip.DutyPeriodsDetails)
                                {
                                    ...
                                    // if (BidLinesBsedonCurrentDutyPeriod.ArrStaLastLeg == "RE")
                                    if (trip.RedEye)
                                    {

                                        bool isInVacation = GlobalSettings.TempOrderedVacationDays.Any(x => x.AbsenceType == "VA" && (x.StartAbsenceDate <= tDate && x.EndAbsenceDate >= tDate));
                                        if (isInVacation)
                                        {
                                            BidLinesBsedonCurrentDutyPeriod.BidLineType = (int)BidLineType.VA;
                                            vacationDutyPeriod.VacationType = "VA";
                                        }
                                        else
                                        {
                                            BidLinesBsedonCurrentDutyPeriod.BidLineType = (int)BidLineType.VO;
                                            vacationDutyPeriod.VacationType = "VO";
                                        }
                                        ...
                                        if (BidLinesBsedonCurrentDutyPeriod.ArrStaLastLeg == "RE")
                                        {
                                            if (isInVacation)
                                            {
                                                VAHelperList.Add(new VAHelper() { StartDate = tDate, EndDate = tDate, VA = 0, IsInBp = true });

                                            }
                                            
                                            tDate = tDate.AddDays(1);

                                            BidLinesBsedonCurrentDutyPeriod = line.BidLineTemplates.FirstOrDefault(x => x.Date == tDate);
                                             isInVacation = GlobalSettings.TempOrderedVacationDays.Any(x => x.AbsenceType == "VA" && (x.StartAbsenceDate <= tDate && x.EndAbsenceDate >= tDate));
                                            if (isInVacation)
                                            {
                                                BidLinesBsedonCurrentDutyPeriod.BidLineType = (int)BidLineType.VA;
                                                vacationDutyPeriod.VacationType = "VA";
                            ...
                            else if (tripType == "VOB")
                            {
                                ...
                                if (vTrip == null) return;
                                vATfp = vTrip.VacationGrandTotal.VA_TfpInBpTot;
                                decimal vATfpInLine = vTrip.VacationGrandTotal.VA_TfpInLineTot;
                                ...
                                if (isVDMTrip)
                                {
                                    var vdmTrip = GlobalSettings.VacationData.Where(x => x.Key == pairing).Select(y => y.Value.VofData).FirstOrDefault();
                                    if (vdmTrip == null) continue;
                                    vATfp = vdmTrip.VacationGrandTotal.VA_TfpInBpTot;
                                    vADays = vdmTrip.DutyPeriodsDetails.Where(x => x.VacationType == "VA").Count();

                                    if (tripEndDate > GlobalSettings.CurrentBidDetails.BidPeriodEndDate)
                                    {

                                        //VAHelperList.Add(new VAHelper() { StartDate = tripEndDate.AddDays(-(vADays - 1)), EndDate = tripStartDate.AddDays(vTrip.DutyPeriodsDetails.Where(x => x.VacationType == "VA" && x.isInBp).Count() - 1), VA = vATfp - (vTrip.VacationGrandTotal.VA_TfpInLineTot - vTrip.VacationGrandTotal.VA_TfpInBpTot) });
                                        //commented above code by Roshil on 06-09-2021 . In october , we have "F" month vacation which starts on 31st and ends on next BP. In this case , I can see "VA" bid line type calculated as "VAP" and modern bid line view display VAP for VA dutyperiod. The reason is that VAPhelper list contains start date 31 and end date 30. 
                                        //VAHelperList.Add(new VAHelper() { StartDate = tripEndDate.AddDays(-(vADays - 1)), EndDate = GlobalSettings.CurrentBidDetails.BidPeriodEndDate, VA = vATfp - (vdmTrip.VacationGrandTotal.VA_TfpInLineTot - vdmTrip.VacationGrandTotal.VA_TfpInBpTot) });
                                        int dpinoutsidebidperiod = vdmTrip.DutyPeriodsDetails.Where(x => x.VacationType == "VA" && !x.isInBp).Count();
                                        if (vdmTrip.DutyPeriodsDetails.Where(x => x.VacationType == "VA" && !x.isInBp).Count() > 0)
                                        {
                                            VAHelperList.Add(new VAHelper() { StartDate = tripEndDate.AddDays(-dpinoutsidebidperiod + 1), EndDate = tripEndDate, VA = vdmTrip.VacationGrandTotal.VA_TfpInLineTot - vdmTrip.VacationGrandTotal.VA_TfpInBpTot, IsInBp = true });
                                ...
                                foreach (VacationDutyPeriod vacationDutyPeriod in vTrip.DutyPeriodsDetails)
                                {
                                    ...
                                    if(trip.RedEye)
                                    {
                                        ...
                                        if (isInVacation)
                                        {
                                            BidLinesBsedonCurrentDutyPeriod.BidLineType = (int)BidLineType.VA;
                                            if (BidLinesBsedonCurrentDutyPeriod.ArrStaLastLeg == "RE")
                                                VAHelperList.Add(new VAHelper() { StartDate = tDate, EndDate = tDate, VA = 0, });
                                        }
                                        else
                                        {
                                            BidLinesBsedonCurrentDutyPeriod.BidLineType = (int)BidLineType.VO;
                ...
                var monthTomonthVacation = GlobalSettings.TempOrderedVacationDays.FirstOrDefault(x => x.EndAbsenceDate > GlobalSettings.CurrentBidDetails.BidPeriodEndDate);

                if (monthTomonthVacation != null)
                {
                    decimal vacationGurantee = Math.Max((monthToMonthVaBp + monthToMonthVainNextBp), ((monthTomonthVacation.EndAbsenceDate - monthTomonthVacation.StartAbsenceDate).Days + 1) * GlobalSettings.DailyVacPay);
                    var totalVAPinMonthToMonthVacation = VAPHelper.Where(x => x.StartDate >= monthTomonthVacation.StartAbsenceDate && x.EndDate <= monthTomonthVacation.EndAbsenceDate).Sum(x => x.VAPValue);
                    line.clawBack = Math.Max(monthToMonthVaBp + monthToMonthVainNextBp + totalVAPinMonthToMonthVacation - vacationGurantee, 0);
                }
            }
        ...
        private List<VAPHelper> GenerateVAPValues(List<VAHelper> lstVAHelper)
        {
            ...
            foreach (Absense vacationPeriod in GlobalSettings.TempOrderedVacationDays)
            {
                ...
                //Roshil commented this code on 5-6-2025 to get the correct VAP outside the next month
                //decimal actualVapInNextBp = actualVap - actualVapInBp;
                int totalVacationDaysInNextBp = totalVacationDays - totalVacationDaysInBP;
                decimal actualVapInNextBp = totalVacationDaysInNextBp * GlobalSettings.DailyVacPay;
                actualVapInNextBp = actualVapInNextBp - (totTfp - totTfpinBp);
                //roshil change end  5-6-2025
                //Roshil add on 18-5-2025 : We need to set VAP zero for all the next BP(For eom vacation only)
                if (vacationPeriod.StartAbsenceDate > GlobalSettings.CurrentBidDetails.BidPeriodEndDate)
                    actualVapInNextBp = 0;
                //end Roshil add on 18-5-2025
